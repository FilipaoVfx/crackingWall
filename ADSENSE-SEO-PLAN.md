...
- [x] 🟢 **Origen de las imágenes de wallpapers (autenticidad) — ACLARADO:** La galería de **producción** siempre usó los **9 wallpapers reales de la DB de Supabase** (arte propio, AI-assisted + upscaled, alojado en Supabase Storage — 0 Unsplash). El Unsplash solo estaba en el **fallback local** (`wallpapers.json`), que se usa únicamente si el build no alcanza Supabase. Corregido: el fallback local ahora **espeja los 9 reales** (sin Unsplash, sin arte inventado). Heroes de los 3 posts viejos → apuntan a wallpapers reales de Supabase. *(La idea previa de generar arte propio se descartó: los reales de la DB son los que se mantienen.)*
